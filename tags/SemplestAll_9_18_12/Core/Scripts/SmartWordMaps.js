...
function showOptionsURL(type, address, city, state, zip) {
    ...
    if (city == ' ' || city == '' || city == null) {
        location.address = $('#Addresses_0__Address').val();
        location.city = $('#Addresses_0__City').val();
        location.state = $("#Addresses_0__StateCodeFK option:selected").text();
...
function renderOptions(response) {
    ...
    if (outFormat == "json") {
        ...
        var proxVal = this.$.find("select[id='Addresses_" + localIndex + "__ProximityRadius']")[0].value;
        ...
        $("#IsState").val("False");
        $("#Addresses_" + localIndex + "__ProximityMessage").text("Required");
        $("#Addresses_" + localIndex + "__ProximityMessage").css("visibility", "hidden");
        $("#getCategories").removeAttr("disabled");
        $("#Addresses_" + localIndex + "__IsCountry").val("False");
        $("#Addresses_" + localIndex + "__IsState").val("False");
        $("input[id='Addresses_" + localIndex + "__Latitude']").val('');
        $("input[id='Addresses_" + localIndex + "__Longitude']").val('');
        if (locations.length > 1 || locations[0].geocodeQuality == 'COUNTRY') {
            lng = -99.141968;
            lat = 39.527596;
            $("#Addresses_" + localIndex + "__IsCountry").val("True");
            zoomval = 3;
        }
        else if ($("#Addresses_" + localIndex + "__StateCodeFK").val() != "--" &&
                    ($("#Addresses_" + localIndex + "__ProximityRadius").val().trim() == '' || $("#Addresses_" + localIndex + "__ProximityRadius").val().trim() == '0') &&
                    $("#Addresses_" + localIndex + "__City").val().trim() == '' &&
                    $("#Addresses_" + localIndex + "__Zip").val().trim() == '' &&
                    $("#Addresses_" + localIndex + "__Address").val().trim() == '') {
            $("#Addresses_" + localIndex + "__ProximityRadius").val("");
            $("#Addresses_" + localIndex + "__IsState").val("True");
            zoomval = 3;
        }
        else if ($("#Addresses_" + localIndex + "__StateCodeFK").val() != "--" &&
                    ($("#Addresses_" + localIndex + "__ProximityRadius").val() != 5) &&
                    $("#Addresses_" + localIndex + "__City").val().trim() == '' &&
                    $("#Addresses_" + localIndex + "__Zip").val().trim() == '' &&
                    $("#Addresses_" + localIndex + "__Address").val().trim() == '') {
            $("#getCategories").attr("disabled", "disabled");
        }
        else if ($("#Addresses_" + localIndex + "__ProximityRadius").val().trim() == '' || $("#Addresses_" + localIndex + "__ProximityRadius").val().trim() == '0') {
            $("#Addresses_" + localIndex + "__ProximityMessage").css("visibility", "visible");
            $("#getCategories").attr("disabled", "disabled");
            zoomval = 3;
        }
        else if (locations[0].geocodeQuality != "STATE") {
            $("input[id='Addresses_" + localIndex + "__Latitude']").val(location.latLng.lat);
            $("input[id='Addresses_" + localIndex + "__Longitude']").val(location.latLng.lng);
        }
        ...
        if (list != null) {
            ...
            if (nextIndex != null)
                doOptions1('Addresses_' + nextIndex + '__Address', 'Addresses_' + nextIndex + '__City', 'Addresses_' + nextIndex + '__StateCodeFK', 'Addresses_' + nextIndex + '__Zip', 'Addresses_' + nextIndex + '__ProximityRadius', nextIndex);
        }
        mapArray.push(map);
        return;
    }
