...
public class MsnCloudServiceImpl implements semplest.services.client.interfaces.MsnAdcenterServiceInterface //MsnCloudService
{
	...
	public MsnManagementIds createAccount(SemplestString name) throws MsnCloudException
	{
		...
		try
		{
			...
			SignupCustomerRequest req = new SignupCustomerRequest();
			req.setCustomer(customer);
			req.setUser(user);
			req.setAccount(account);
			req.setParentCustomerId(adCenterCredentials.getParentCustomerID());
			req.setApplicationScope(ApplicationType.Advertiser);
			SignupCustomerResponse signupCustomerResponse = customerManagementService.signupCustomer(req);
	...
	public Long getCustomerID(String name) throws MsnCloudException{
		try
		{
			...
			CustomerInfo[] acInf = signupCustomerResponse.getCustomersInfo().getCustomerInfo();
			if(acInf==null) {
	...
	public HashMap<String,Double> getAccountIDs() throws MsnCloudException{
		try
		{
			...
			AccountInfo[] acInf = signupCustomerResponse.getAccountsInfo().getAccountInfo();
	...
	public Account getAccountById(Long accountId) throws MsnCloudException
	{
		...
		{
			ICustomerManagementService customerManagementService = getCustomerManagementService();
			GetAccountRequest getAccReq = new GetAccountRequest();
			getAccReq.setAccountId(accountId);
			account = customerManagementService.getAccount(getAccReq);
	...
	{	
		...
		try {
			ret = createCampaign(new Long(data.get("accountId")), 
					...
					gson.fromJson(data.get("CampaignStatus"), CampaignStatus.class));
		} catch (MsnCloudException e) {
			throw new Exception(e);
		}
		return gson.toJson(ret);
			...
			throws MsnCloudException
	{
		try
		{
			...
			AddCampaignsRequest req = new AddCampaignsRequest();
			req.setAccountId(accountId);
			ArrayOfCampaign cps = new ArrayOfCampaign();
			cps.setCampaign(campaign);
			req.setCampaigns(cps);
			addCampaigns = campaignManagement.addCampaigns(req);
			return addCampaigns.getCampaignIds().get_long()[0];
		}
		catch (RemoteException e)
		{
			throw new MsnCloudException(e);
	...
	public Campaign getCampaignById(Long accountId, Long campaignId) throws RemoteException
	{
		...
		ICampaignManagementService campaignManagement = getCampaignManagementService(accountId);
		GetCampaignsByIdsRequest req = new GetCampaignsByIdsRequest();
		req.setAccountId(accountId);
		ArrayOflong ids = new ArrayOflong();
		ids.set_long(new long[]	{ campaignId });
		req.setCampaignIds(ids);
		campaignsById = campaignManagement.getCampaignsByIds(req);
		
		return campaignsById.getCampaigns().getCampaign()[0];
	}
	...
	public Campaign[] getCampaignsByAccountId(Long accountId) throws RemoteException
	{
		...
		req.setAccountId(accountId);
		GetCampaignsByAccountIdResponse campaigns = campaignManagement.getCampaignsByAccountId(req);
			
		return campaigns.getCampaigns().getCampaign();
	}
	...
	{
		ICampaignManagementService campaignManagement = getCampaignManagementService(accountId);
		PauseCampaignsRequest req = new PauseCampaignsRequest();
		req.setAccountId(accountId);
		ArrayOflong ids = new ArrayOflong();
		ids.set_long(new long[] { campaignId });
		req.setCampaignIds(ids);
		campaignManagement.pauseCampaigns(req);
	}
	...
	{
		ICampaignManagementService campaignManagement = getCampaignManagementService(accountId);
		ResumeCampaignsRequest req = new ResumeCampaignsRequest();
		req.setAccountId(accountId);
		ArrayOflong ids = new ArrayOflong();
		ids.set_long(new long[] { campaignId });
		req.setCampaignIds(ids);
		campaignManagement.resumeCampaigns(req);
	}
	...
	{
		ICampaignManagementService campaignManagement = getCampaignManagementService(accountId);
		DeleteCampaignsRequest req = new DeleteCampaignsRequest();
		req.setAccountId(accountId);
		ArrayOflong ids = new ArrayOflong();
		ids.set_long(new long[] { campaignId });
		req.setCampaignIds(ids);
		campaignManagement.deleteCampaigns(req);
	}
	...
	public void setCampaignStateTargets(Long accountId, long customerId, Long campaignId, List<String> states) throws RemoteException
	{
		...
		SetTargetToCampaignRequest setTargetToCampaignRequest = new SetTargetToCampaignRequest();
		setTargetToCampaignRequest.setCampaignId(campaignId);
		setTargetToCampaignRequest.setTargetId(statesTargetId);
	...
	public void deleteCampaignTargets(Long accountId, long customerId, Long campaignId) throws RemoteException
	{
			...
			DeleteTargetFromCampaignRequest dfc = new DeleteTargetFromCampaignRequest();
			dfc.setCampaignId(campaignId);
			campaignManagement.deleteTargetFromCampaign(dfc);
			DeleteTargetsFromLibraryRequest deleteTargetsFromLibraryRequest = new DeleteTargetsFromLibraryRequest();
			ArrayOflong ids = new ArrayOflong();
			ids.set_long(new long[] { campaignTargets.getId() });
			deleteTargetsFromLibraryRequest.setTargetIds(ids);
	...
	{		
		...
		long[] campaignIds =
		{ campaignId };
		GetTargetsByCampaignIdsRequest getTargetsByCampaignIdsRequest = new GetTargetsByCampaignIdsRequest();
		ArrayOflong ids = new ArrayOflong();
		ids.set_long(campaignIds);
		getTargetsByCampaignIdsRequest.setCampaignIds(ids);
		...
		Target[] targets = targetsByCampaignIds.getTargets().getTarget();
	...
	public void updateCampaignBudget(Long accountId, Long campaignId, BudgetLimitType budgetLimitType, double dailyBudget, double monthlyBudget) throws RemoteException
	{
		...
		ArrayOfCampaign arr = new ArrayOfCampaign();
		arr.setCampaign(campaigns);
		updateCampaignsRequest.setCampaigns(arr);
	...
	@Override
	public long createAdGroup(Long accountId, Long campaignId) throws RemoteException
	{
		ICampaignManagementService campaignManagement = getCampaignManagementService(accountId);
		String nextAdGroupName = uniqueMsnNameService.getNextAdGroupName();
		AdGroup build = aNew().adGroup().withName(nextAdGroupName).build();
		AddAdGroupsRequest req = new AddAdGroupsRequest();
		req.setCampaignId(campaignId);
		ArrayOfAdGroup arr = new ArrayOfAdGroup();
		arr.setAdGroup( new AdGroup[] { build });
		req.setAdGroups(arr);
		AddAdGroupsResponse addAdGroups = campaignManagement.addAdGroups(req);
		
		final Long adGroupId = addAdGroups.getAdGroupIds().get_long()[0];
	...
	public Boolean updateAdGroupDefaultBids(Long accountId, Long campaignId, Long adGroupId, 
			Double exactMatchBid, Double phraseMatchBid, Double broadMatchBid) throws RemoteException{
		ICampaignManagementService campaignManagement = getCampaignManagementService(accountId);
		...
		AdGroup[] adGroups = adGroupResp.getAdGroups().getAdGroup();
		Bid broadMBid = new Bid();
		broadMBid.setAmount(broadMatchBid);
		Bid exactMBid = new Bid();
		exactMBid.setAmount(exactMatchBid);
		Bid phraseMBid = new Bid();
		phraseMBid.setAmount(phraseMatchBid);
		...
		ArrayOfAdGroup arr = new ArrayOfAdGroup();
		arr.setAdGroup(adGroups);
		updateReq.setAdGroups(arr);
	...
	public AdGroup[] getAdGroupsByCampaignId(Long accountId, Long campaignId) throws RemoteException
	{
		...
		GetAdGroupsByCampaignIdRequest req = new GetAdGroupsByCampaignIdRequest();
		req.setCampaignId(campaignId);
		adGroups = campaignManagement.getAdGroupsByCampaignId(req);	
		return adGroups.getAdGroups().getAdGroup();
	}
	...
	public AdGroup getAdGroupById(Long accountId, Long campaignId, Long adGroupId) throws RemoteException
	{
		...
		GetAdGroupsByIdsRequest req = new GetAdGroupsByIdsRequest();
		req.setCampaignId(campaignId);
		ArrayOflong arr = new ArrayOflong();
		arr.set_long(new long[] { adGroupId });
		req.setAdGroupIds(arr);
		adGroupsByIds = campaignManagement.getAdGroupsByIds(req);
		return adGroupsByIds.getAdGroups().getAdGroup()[0];
	}
	...
	{
		ICampaignManagementService campaignManagement = getCampaignManagementService(accountId);
		DeleteAdGroupsRequest req = new DeleteAdGroupsRequest();
		req.setCampaignId(campaignId);
		ArrayOflong arr = new ArrayOflong();
		arr.set_long(new long[] { adGroupId });
		req.setAdGroupIds(arr);
		campaignManagement.deleteAdGroups(req);
	}
