			...
			try {
				...
				for(SERVER server : SERVER.values()){
					if(monitor.isMonitorServer(server)){
						...
						SemplestBiddingServiceClient biddingClient = new SemplestBiddingServiceClient(esbUrl, monitor.getClientTimeout());
						
						try {
							String ret = biddingClient.checkStatus(monitor.getClientTimeout());
