...
public class AdengineServiceTest extends BaseDB
{
	...
	public int Test_Adengine_Client()
	{
		...
		try
		{
			 ...
			 * In progress:
			 * 
			 * UnpausePromotion(); //google,msn - n/a for test ChangePromotionStartDate(); //low priority
			 */
		}
		catch (Exception e)
		{
	...
	private void AddPromotionToAdEngine(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			if (testData.adEngineList.contains(AdEngine.Google))
			{
				...
				sql = "select aep.AdvertisingEngineCampaignPK from AdvertisingEnginePromotion aep " + "inner join AdvertisingEngineAccount aea on aep.AdvertisingEngineAccountFK = aea.AdvertisingEngineAccountPK " + "where aea.AdvertisingEngineFK = 2 and aep.PromotionFK = ?";
				testData.googleCampaignId = jdbcTemplate.queryForLong(sql, testData.semplestPromotionId);

				sql = "SELECT aep.AdvertisingEngineAdGroupID FROM AdvertisingEnginePromotion aep " + "inner join AdvertisingEngineAccount aea on aep.AdvertisingEngineAccountFK = aea.AdvertisingEngineAccountPK " + "where aea.AdvertisingEngineFK = 2 and aep.PromotionFK = ?";
				...
				if (match != 1)
				{
					errorHandler(new Exception(vMsg + "Ads are not created correctly on google. " + "An Ad title '" + finaATitle + "' is not found."));
				}
				if (gads.length != testData.promotionAds.size())
				{
					// verify the num of Ads get added
					errorHandler(new Exception(vMsg + "The num of Ads in database and the num of Ads be created on google don't match. " + "Num of active Ads in database: " + testData.promotionAds.size() + ". Num of Ads added to google: " + gads.length));
				}
				...
				if (match != 1)
				{
					errorHandler(new Exception(vMsg + "Keywords are not created correctly on google. " + "An Keyword '" + findAKeyword + "' is not found."));
				}
				if (gkws.length != testData.keywords.size())
				{
					// verify the num of Keywords get added
					errorHandler(new Exception(vMsg + "Not all Keywords in database are created on google. " + "Num of Keywords in database: " + testData.keywords.size() + ". Num of Keywords added to google: " + gkws.length));
				}
				...
				if (match != 1)
				{
					errorHandler(new Exception(vMsg + "Negative Keywords are not created correctly on google. " + "A Negative Keyword '" + findANegKeyword + "' is not found."));
				}
			...
			if (testData.adEngineList.contains(AdEngine.MSN))
			{
				...
				sql = "select aep.AdvertisingEngineCampaignPK from AdvertisingEnginePromotion aep " + "inner join AdvertisingEngineAccount aea on aep.AdvertisingEngineAccountFK = aea.AdvertisingEngineAccountPK " + "where aea.AdvertisingEngineFK = 1 and aep.PromotionFK = ?";
				testData.msnCampaignId = jdbcTemplate.queryForLong(sql, testData.semplestPromotionId);

				sql = "SELECT aep.AdvertisingEngineAdGroupID FROM AdvertisingEnginePromotion aep " + "inner join AdvertisingEngineAccount aea on aep.AdvertisingEngineAccountFK = aea.AdvertisingEngineAccountPK " + "where aea.AdvertisingEngineFK = 1 and aep.PromotionFK = ?";
				...
				if (match != 1)
				{
					errorHandler(new Exception(vMsg + "Ads are not created correctly on MSN. " + "An Ad title '" + finaATitle + "' is not found."));
				}
				if (mads.length != testData.promotionAds.size())
				{
					// verify the num of Ads get added
					errorHandler(new Exception(vMsg + "The num of Ads in database and the num of Ads be created on MSN don't match. " + "Num of active Ads in database: " + testData.promotionAds.size() + ". Num of Ads added to google: " + mads.length));
				}
				...
				if (match != 1)
				{
					errorHandler(new Exception(vMsg + "Keywords are not created correctly on MSN. " + "An Keyword '" + findAKeyword + "' is not found."));
				}
				if (mkws.length != testData.keywords.size())
				{
					// verify the num of Keywords get added
					errorHandler(new Exception(vMsg + "Not all Keywords in database are created on MSN. " + "Num of Keywords in database: " + testData.keywords.size() + ". Num of Ads added to MSN: " + mkws.length));
				}
				...
				/*
				 * Double mLatitude = mLocation.getRadiusTarget().getBids()[0].getLatitudeDegrees(); Double mLongitude =
				 * mLocation.getRadiusTarget().getBids()[0].getLongitudeDegrees(); int mRadius = mLocation.getRadiusTarget().getBids()[0].getRadius();
				 * 
				 * System.out.println(" -address = (street address), " + mLocation.getCityTarget().getBids()[0].getCity() + ", " +
				 * mLocation.getMetroAreaTarget().getBids()[0].getMetroArea() + ", " + mLocation.getStateTarget().getBids()[0].getState() + ", " +
	...
	private void UpdateGeoTargeting(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			jdbcTemplate.update(sql, new Object[] { testData.newAddress, testData.newCity, testData.newStateCode, testData.newZipCode, testData.newLongitude, testData.newLatitude, testData.newRadius, testData.semplestPromotionId });
			...
			if (testData.adEngineList.contains(AdEngine.MSN))
			{
				...
				// TODO
				/*
				 * Double mLatitude = mTarget.getLocation().getRadiusTarget().getBids()[0].getLatitudeDegrees(); Double mLongitude =
				 * mTarget.getLocation().getRadiusTarget().getBids()[0].getLongitudeDegrees(); int mRadius =
				 * mTarget.getLocation().getRadiusTarget().getBids()[0].getRadius();
				 * 
				 * System.out.println(" -address = (street address), " + mLocation.getCityTarget().getBids()[0].getCity() + ", " +
				 * mLocation.getMetroAreaTarget().getBids()[0].getMetroArea() + ", " + mLocation.getStateTarget().getBids()[0].getState() + ", " +
				 * mLocation.getCountryTarget().getBids()[0].getCountryAndRegion()); System.out.println(" -latitude = " + mLatitude);
				 * System.out.println(" -longtitude = " + mLongitude); System.out.println(" -radius = " + mRadius);
				 * 
				 * if(!mLongitude.equals(testData.newLongitude)){ errorHandler(new Exception(vMsg + "Longitude doesn't match. Value in database = " +
				 * testData.newLongitude + ". Value on msn = " + mLongitude + ".")); } if(!mLatitude.equals(testData.newLatitude)){ errorHandler(new
				 * Exception(vMsg + "Latitude doesn't match. Value in database = " + testData.newLatitude + ". Value on msn = " + mLatitude + ".")); }
				 * if(mRadius != testData.newRadius.intValue()){ errorHandler(new Exception(vMsg + "Latitude doesn't match. Value in database = " +
				 * testData.newRadius + ". Value on msn = " + mRadius + "(double to int).")); }
				 */
			}
	...
	private void UpdateBudget(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			/*
			 * //get the BudgetToAddToNextCycle from database String sql = "SELECT P.BudgetToAddToNextCycle FROM Promotion P WHERE PromotionPK = ?";
			 * updatedBudget = jdbcTemplate.queryForObject(sql, new Object[] {testData.semplestPromotionId}, Double.class);
			 * 
			...
			if (testData.adEngineList.contains(AdEngine.Google))
			{
				...
				for (SitelinksExtension sle : gSiteLinks)
				{
					...
					for (com.google.api.adwords.v201109_1.cm.Sitelink sl : sle.getSitelinks())
					{
						...
						if ((sl.getDisplayText().equalsIgnoreCase(testData.siteLink3.linkText)) && (sl.getDestinationUrl().equalsIgnoreCase(testData.siteLink3.linkUrl)))
						{
							match++;
						}
					}
					...
					{
						errorHandler(new Exception(vMsg + "The num of SiteLinks in database and the num of SiteLinks be refreshed on google don't match. " + "Num of SiteLinks in database: " + testData.sitelinks.size() + ". Num of SiteLinks refreshed to google: " + sle.getSitelinks().length));
					}
					if (match != 1)
					{
						errorHandler(new Exception(vMsg + "SiteLinks are not refreshed correctly on google. " + "A SiteLink " + testData.siteLink3 + " is not found on google."));
					}
				}
			}
	...
	private void AddAds(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			{
				String sql = "INSERT INTO PromotionAds(PromotionFK,AdTitle,AdTextLine1,AdTextLine2,IsDeleted,CreatedDate) " + "VALUES(?, ?, ?, ?, 0 , CURRENT_TIMESTAMP)";
				jdbcTemplate.update(sql, new Object[] { testData.semplestPromotionId, ad.adTitle, ad.adTextLine1, ad.adTextLine2 });

				sql = "SELECT pa.PromotionAdsPK FROM PromotionAds pa WHERE pa.PromotionFK = ? and pa.AdTitle = ?";
				Integer id = jdbcTemplate.queryForInt(sql, new Object[] { testData.semplestPromotionId, ad.adTitle });
			...
			if (testData.adEngineList.contains(AdEngine.Google))
			{
				...
				{
					// verify the num of Ads get added
					errorHandler(new Exception(vMsg + "The num of Ads in database and the num of Ads be created on google don't match. " + "Num of active Ads in database: " + numAllAds + ". Num of Ads added to google: " + gads.length));
				}
			}
			...
			if (testData.adEngineList.contains(AdEngine.MSN))
			{
				...
				{
					// verify the num of Ads get added
					errorHandler(new Exception(vMsg + "The num of Ads in database and the num of Ads be created on MSN don't match. " + "Num of active Ads in database: " + numAllAds + ". Num of Ads added to google: " + mads.length));
				}
			}
	...
	private void UpdateAds(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			jdbcTemplate.update(sql, new Object[] { updateAd.adTitle, updateAd.adTextLine1, updateAd.adTextLine2, testData.newAdIds.get(0) });
			...
			if (testData.adEngineList.contains(AdEngine.Google))
			{
				...
				if (match != 1)
				{
					errorHandler(new Exception(vMsg + "Ads are not updated correctly on google. " + "An Ad title '" + finaATitle + "' is not found."));
				}
			}
			...
			if (testData.adEngineList.contains(AdEngine.MSN))
			{
				...
				if (match != 1)
				{
					errorHandler(new Exception(vMsg + "Ads are not updated correctly on MSN. " + "An Ad title '" + finaATitle + "' is not found."));
				}
			}
	...
	private void DeleteAds(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			if (testData.adEngineList.contains(AdEngine.Google))
			{
				...
				if (match > 0)
				{
					errorHandler(new Exception(vMsg + "Ads are not completely deleted on google. " + "An Ad title '" + finaATitle + "' is found, while it should have been deleted."));
				}
			}
			...
			if (testData.adEngineList.contains(AdEngine.MSN))
			{
				...
				if (match > 0)
				{
					errorHandler(new Exception(vMsg + "Ads are not completely deleted on MSN. " + "An Ad title '" + finaATitle + "' is found, while it should have been deleted."));
				}
			}
	...
	private void AddKeywords(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			{
				String sql = "INSERT INTO PromotionKeywordAssociation(KeywordFK,PromotionFK,CreatedDate,IsActive,IsDeleted,IsNegative,SemplestProbability,IsTargetMSN,IsTargetGoogle)" + "VALUES(?,?,CURRENT_TIMESTAMP,1,0,0,1,1,1)";

				jdbcTemplate.update(sql, new Object[] { kwId, testData.semplestPromotionId });
			}
			...
			if (testData.adEngineList.contains(AdEngine.Google))
			{
				...
				if (match != 1)
				{
					errorHandler(new Exception(vMsg + "Keywords are not added correctly on google. " + "An Keyword '" + findAKeyword + "' is not found."));
				}
				if (gkws.length != numAllKeywords)
				{
					// verify the num of Keywords get added
					errorHandler(new Exception(vMsg + "Not all Keywords in database are added to google. " + "Num of Keywords in database: " + numAllKeywords + ". Num of Ads added to google: " + gkws.length));
				}
			}
			...
			if (testData.adEngineList.contains(AdEngine.MSN))
			{
				...
				if (match != 1)
				{
					errorHandler(new Exception(vMsg + "Keywords are not added correctly on MSN. " + "An Keyword '" + findAKeyword + "' is not found."));
				}
				if (mkws.length != numAllKeywords)
				{
					// verify the num of Keywords get added
					errorHandler(new Exception(vMsg + "Not all Keywords in database are added to MSN. " + "Num of Keywords in database: " + numAllKeywords + ". Num of Ads added to MSN: " + mkws.length));
				}
			}
	...
	private void DeleteKeywords(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			if (testData.adEngineList.contains(AdEngine.Google))
			{
				...
				if (match > 0)
				{
					errorHandler(new Exception(vMsg + "Keywords are not completely deleted on google. " + "A Keyword '" + findAKeyword + "' is found, while it should have been deleted."));
				}
			}
			...
			if (testData.adEngineList.contains(AdEngine.MSN))
			{
				...
				if (match > 0)
				{
					errorHandler(new Exception(vMsg + "Keywords are not completely deleted on MSN. " + "A Keyword '" + findAKeyword + "' is found, while it should have been deleted."));
				}
			}
	...
	private void AddNegativeKeywords(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			for (Integer kwid : testData.newNegKeywordIds)
			{
				...
				String sql = "INSERT INTO PromotionKeywordAssociation(KeywordFK,PromotionFK,CreatedDate,IsActive,IsDeleted,IsNegative,SemplestProbability,IsTargetMSN,IsTargetGoogle)" + "VALUES(?,?,CURRENT_TIMESTAMP,1,0,1,1,1,1)";
	...
	private void DeleteNegativeKeywords(TEST_METHOD method) throws Exception
	{
		...
		try
		{
			...
			if (testData.adEngineList.contains(AdEngine.Google))
			{
				...
				{
					errorHandler(new Exception(vMsg + "The num of Negative Keywords in database and the num of Negative Keywords put on google don't match. " + "Num of Negative Keywords active in database: " + numExpectedOutputNegKeywords + ". Num of Negative Keywords existing to google: "
							+ negKeywords.size()));
				}
				if (match > 0)
				{
	...
	private boolean initializeTest()
	{
		try
		{
			...
			sql = "INSERT INTO Promotion (ProductGroupFK,PromotionName,PromotionDescription,PromotionStartDate,PromotionEndDate," + "LandingPageURL,DisplayURL,PromotionBudgetAmount,BudgetCycleFK,CycleStartDate,CycleEndDate,StartBudgetInCycle,RemainingBudgetInCycle,BudgetToAddToNextCycle,"
					+ "IsPaused,IsCompleted,IsLaunched,CreatedDate,IsDeleted)" + "VALUES (?, ?, 'wedding dresses, gowns, brides, flowers, cakes, hair, booze, food', CURRENT_TIMESTAMP, '2013-01-01', "
					+ "'http://www.weddingchannel.com', 'http://www.weddingchannel.com',300.00, 3, CURRENT_TIMESTAMP, '2013-01-01', 500.00, 500.00, 300.00, " + "0, 0, 1, CURRENT_TIMESTAMP, 0)";
			jdbcTemplate.update(sql, new Object[] { testData.semplestProductGroupId, testData.semplestPromotionName });

			sql = "SELECT p.PromotionPK FROM Promotion p WHERE PromotionName = ?";
			testData.semplestPromotionId = jdbcTemplate.queryForInt(sql, new Object[] { testData.semplestPromotionName });
			...
			testData.promotionAdIds = jdbcTemplate.queryForList(sql, new Object[] { testData.semplestPromotionId }, Integer.class);
			...
			jdbcTemplate.update(sql, new Object[] { testData.semplestPromotionId, testData.address, testData.city, testData.stateCode, testData.zipCode, testData.longitude, testData.latitude, testData.radius });

			// SiteLinks
			for (TestDataModel.SiteLink sl : testData.sitelinks)
			{
				sql = "INSERT INTO SiteLinks(PromotionFK,LinkText,LinkURL) " + "VALUES(?,?,?)";
				jdbcTemplate.update(sql, new Object[] { testData.semplestPromotionId, sl.linkText, sl.linkUrl });
			}
	...
	private void clearHistoryTestData()
	{
		try
		{
			...
			sql = "delete from ScheduleLog; " + "delete from ScheduleJob; " + "delete from ScheduleTaskAssociation; " + "delete from Schedule; " + "delete from Task; ";
			jdbcTemplate.update(sql);

			System.out.println("  -- clear other data");
			sql = "DELETE from KeywordBidData;" + "DELETE FROM TargetedDailyBudget;" + "DELETE FROM AdvertisingEngineAds;";
			jdbcTemplate.update(sql);

			// get the list of promotion IDs
			sql = "SELECT p.PromotionPK FROM Promotion p WHERE ProductGroupFK = ?";
			List<Integer> promoIds = jdbcTemplate.queryForList(sql, new Object[] { testData.semplestProductGroupId }, Integer.class);
			// {testData.productGroupIds.get(1)}, Integer.class);

			System.out.println("  -- delete promotion data.");
			for (Integer promoId : promoIds)
			{
				sql = "DELETE FROM AdvertisingEnginePromotion where PromotionFK = ?;" + "DELETE PromotionAds WHERE PromotionFK = ?;" + "DELETE FROM KeywordBid WHERE PromotionFK = ?;" + "DELETE PromotionKeywordAssociation WHERE PromotionFK = ?;" + "DELETE GeoTargeting WHERE PromotionFK = ?;"
						+ "DELETE SiteLinks WHERE PromotionFK = ?;" + "DELETE Promotion WHERE PromotionPK = ?;";

				jdbcTemplate.update(sql, new Object[] { promoId, promoId, promoId, promoId, promoId, promoId, promoId });
