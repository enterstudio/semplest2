...
public class DatabaseTest extends BaseDB
{
	...
	public int Test_ALL()
	{
		try
		{
			...
			/*
			 * //Create Test Promotion try{ int ProductGroupFK = 81; String PromotionName = "Database Test"; String PromotionDescription =
			 * "should be deleted after the test"; Date PromotionStartDate = new Date(); String LandingPageURL = "www.semplest.com"; Double
	...
	public void store_ReportData(int caseNum)
	{
		try
		{
			...
			switch (caseNum)
			{
				...
				case 2:
				{// duplicated both keywords and transaction date
	...
	public void get_TrafficEstimatorData(int caseNum)
	{
		try
		{
			...
			{
				case 1:
				{// getLatestTrafficEstimatorForKeyword()
	...
	public void Test_PromotionData()
	{
		try
		{
			...
			try
			{
				...
				// --- verification
				System.out.println("AccountID = " + adEngineId.getAccountID());
				System.out.println("CampaignID = " + adEngineId.getCampaignID());
				System.out.println("AdGroupID = " + adEngineId.getAdGroupID());
			...
			/*
			 * try{ //*** get AdEngine campaign from the database System.out.println("------------------------------------------------------------");
			...
			/*
			 * BudgetObject b = db.getBudget(promotionID); System.out.println("RemainingBudgetInCycle = " + b.getRemainingBudgetInCycle());
	...
	public void Test_Other()
	{
		try
		{
			...
			/*
			 * GetKeywordForAdEngineSP getKeywords = new GetKeywordForAdEngineSP(); List<KeywordProbabilityObject> keywordList =
			 * getKeywords.execute(promotionID,true,false); int i = 0; for(KeywordProbabilityObject k : keywordList){
			 * System.out.println("#"+i+"----------------------------"); System.out.println("keyword = " + k.getKeyword());
			 * System.out.println("probability = " + k.getSemplestProbability()); System.out.println("isActive = " + k.getIsActive());
			 * System.out.println("isTargetGoogle = " + k.getIsTargetGoogle()); System.out.println("isTargetMsn = " + k.getIsTargetMSN()); i++; }
			 */

			/* ******************************************************************************************* */
			// *** Test GetAllPromotionDataSP(). Get promotion data from the database
			/*
			 * GetAllPromotionDataSP retpromo = new GetAllPromotionDataSP(); boolean ret = retpromo.execute(promotionID); PromotionObj pd =
	...
	private void insertKeywordBidTable(ArrayList<Integer> pklist)
	{
		...
		{
			String strSQL = "insert into KeywordBid(KeywordFK, AdvertisingEngineFK, PromotionFK, StartDate, IsActive, BidTypeFK, MicroBidAmount, IsDefaultValue, CreatedDate) " + "VALUES (?,?,?,?,?,?,?,?,?)";

			jdbcTemplate.update(strSQL, new Object[] { keywordPK, 2, promotionID, new Date(), 0, 1, 100000, 0, new Date() });
	...
	private void insertKeywordTable(String[] keywords)
	{
		...
		for (String k : keywords)
		{
			String strSQL = "insert into Keyword(Keyword) " + "VALUES (?)";

			jdbcTemplate.update(strSQL, new Object[] { k });
		}
	}
	...
	private void insertPKtoAssociationTable(ArrayList<Integer> pklist)
	{
		for (Integer keywordPK : pklist)
		{
			String strSQL = "insert into PromotionKeywordAssociation(KeywordFK, PromotionFK, CreatedDate, IsActive, IsDeleted, IsNegative, IsTargetMSN, IsTargetGoogle) " + "VALUES (?,?,?,?,?,?,?,?)";

			jdbcTemplate.update(strSQL, new Object[] { keywordPK, promotionID, new Date(), 0, 0, 0, 0, 1 });
	...
	private ReportObject[] getReportFromGoogle()
	{
		try
		{
			...
			String REPORT_DEFINITION = "<reportDefinition><selector><fields>Date</fields>" + "<fields>AdGroupId</fields><fields>Id</fields><fields>KeywordText</fields><fields>KeywordMatchType</fields>"
					...
					+ "<fields>ApprovalStatus</fields><fields>CampaignId</fields>" + "</selector><reportName>KEYWORDS_PERFORMANCE_REPORT</reportName>" + "<reportType>KEYWORDS_PERFORMANCE_REPORT</reportType>" + "<dateRangeType>ALL_TIME</dateRangeType><downloadFormat>CSV</downloadFormat>"
					+ "</reportDefinition>";
