			...
			try {
				...
				for(SERVER server : SERVER.values()){
					if(monitor.isMonitorServer(server)){
						...
						SemplestSchedulerServiceClient schedulerService = new SemplestSchedulerServiceClient(esbUrl);
						
						try {
							String ret = schedulerService.checkStatus(monitor.getClientTimeout());
