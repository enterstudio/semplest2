...
public class KeywordMonitorThread implements Runnable {

	private long sleep_time;
	private HashMap<SERVER, MonitorData> monitorData;
	private boolean stop = false;	
	
	private SERVICE service = SERVICE.Keyword;
	private String clientTimeout = "30000"; 
	private Notification alert = new Notification();

	public KeywordMonitorThread(long interval_min, HashMap<SERVER, MonitorData> monitorDataTemplate) {
		...
		while(!stop){	
			try {
				for(SERVER s : SERVER.values()){
					...
					KeywordLDAServiceClient keywordService = new KeywordLDAServiceClient(esbUrl);
					try {
						String ret = keywordService.checkStatus(clientTimeout);
