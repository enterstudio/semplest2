			...
			try {
				...
				for(SERVER server : SERVER.values()){
					if(monitor.isMonitorServer(server)){
						...
						SemplestBiddingServiceClient biddingService = new SemplestBiddingServiceClient(esbUrl, monitor.getClientTimeout());
						
						try {
							String ret = biddingService.checkStatus(monitor.getClientTimeout());
