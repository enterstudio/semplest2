package semplest.db.berkeleydb;
 ...
 */
public class BerkeleyDB_Static
{

	// - Privates ------------
	private static String DIR = "/semplest/data/dmoz/BerkeleyDB_Static/";	
	...
	public static int add(String id, Map<String, String> kvs) throws Exception
	{
		final Environment e = getEnv(false);
		final Database db = getDB(id, true, e);
		int count = 0;
		for (Map.Entry<String, String> en : kvs.entrySet())
		{
			final DatabaseEntry key = new DatabaseEntry(en.getKey().getBytes("UTF-8"));
			final DatabaseEntry val = new DatabaseEntry(en.getValue().getBytes("UTF-8"));
	...
	{
		final Environment e = getEnv(false);
		final Database db = getDB(id, false, e);
		final DatabaseEntry key = new DatabaseEntry(k.getBytes("UTF-8"));
	...
	public static Map<String, String> get(String id, String[] ks) throws Exception
	{
