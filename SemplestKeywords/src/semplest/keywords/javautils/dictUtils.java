...
public class dictUtils
{
	...
	/*
	 * // static methods to load paths from property file static { if(SEMplestService.properties==null){ try{ String PROPSFILE =
	 * "../SemplestServices/bin/system.properties"; SEMplestService.properties = new Properties(); FileInputStream is; is = new
	 * FileInputStream(PROPSFILE); SEMplestService.properties.load(is); is.close(); } catch (Exception e){logger.error("Problem", e);} } dictfile=
	 * SEMplestService.properties.getProperty("data.stemworddict"); docfile = SEMplestService.properties.getProperty("data.dmoz.allcats"); twfile =
	 * SEMplestService.properties.getProperty("data.dmoz.alltw");;
	 * 
	 * }
	 */
	...
	{
		return dict.containsKey(getRoot(word)) && !cw.contains(word);
	}
