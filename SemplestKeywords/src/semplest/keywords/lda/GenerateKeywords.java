	...
	public static void main(String[] args) throws Exception {
	  ...
	  /* //----------------------------------------------------------------------
	    ...
	    for(String keys : keyS){
	    	if(i>=numWords)break;
	    	System.out.println(wordM.get(keys)+"\t"+keys);
	    	i++;
	    }
	    //---------------------------------------------------------------------	*/	
	    ...
	    while(itArray.get(0).hasNext() && i<numWords){
	    	for(int j=0; j<numCateg+1; j++) key[j] =itArray.get(j).next();
	    	for(int j=0; j<numCateg+1; j++){
	    		if(key[j].length()<=7) System.out.print(wordMap.get(0).get(key[j])+"\t\t"+key[j]+"\t\t");
	    		else System.out.print(wordMap.get(0).get(key[j])+"\t\t"+key[j]+"\t");
	    	}
		    System.out.print("\n");
		    i++;
	    }
	    ...
	    while(i<numWords){
	    	for(int j=0; j<numCateg+1; j++) {
	    		if(itArray.get(j).hasNext())key[j] =itArray.get(j).next();
	    		else key[j]=null;
	    	}
	    	for(int j=0; j<numCateg+1; j++){
	    		if(key[j]!=null && key[j].length()<=7) System.out.print(wordMap.get(j).get(key[j])+"\t\t"+key[j]+"\t\t");
	    		else {
	    			if(key[j] !=null)System.out.print(wordMap.get(j).get(key[j])+"\t\t"+key[j]+"\t");
	    			else System.out.print("null\t\t"+"\t\t"+key[j]+"\t\t");
	    		}
	    	}
		    System.out.print("\n");
		    i++;
