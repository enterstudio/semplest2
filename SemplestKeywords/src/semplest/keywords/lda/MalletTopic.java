...
public class MalletTopic {
	...
	public double[] calcFscore(InstanceList testInst,int numIter) throws FileNotFoundException{
		...
		double tp=0; double fp=0; double fn=0; 
	...
	public Instance getStringInstancefromFeatureSequenceInstance(Instance inst){
		...
		if(instaux.isLocked()) instaux.unLock();
...
// Creates Instances from a file in SEMplest format
	...
	public void CreateInstancesSEMplestCache(String[] directories) throws Exception {
        ...
        instances = new InstanceListCache (new SerialPipes(pipeList));
	...
	public void CreateInstancesSEMplestCache2(String[] directories) throws Exception {
		// Creates Instances from a file in SEMplest format uses less memory than version 1
	...
	public void CreateInstancesSEMplestCacheMultiThread(String[] directories, int numThreads) throws Exception {
        ...
        if ( directories.length > 0 ) file = directories[0];
        ArrayList<String> lines = ioUtils.readFile( file );
        int numLinesIter = lines.size()/numThreads;
        // Create multiple threads to import all the lines;
        for( int i=0; i<numThreads; i++ ){
	        if( i==numThreads-1){
	        	MalletThreadObj threadObj = new MalletThreadObj(instances, 0, i*numLinesIter, (i+1)*numLinesIter+(lines.size()%numThreads)-1, lines);
	        	new Thread(threadObj).start();
	        } else {
	        	MalletThreadObj threadObj = new MalletThreadObj(instances, 0, i*numLinesIter, (i+1)*numLinesIter-1, lines);
	        	new Thread(threadObj).start();
	        }
        }
...
*/
	...
	public InstanceList CreateInferInstfromFile(File testFile) throws UnsupportedEncodingException, FileNotFoundException{
        ...
        testing.addThruPipe(new CsvIterator (fileReader, Pattern.compile("^(\\S*)[\\s,]*(\\S*)[\\s,]*(.*)$"),
                                               3, 2, 1)); // data, label, name fields
        return testing;
	}
	...
	public void saveLDAInstCache(File serializedFile) throws IOException {
		...
		for(int i=0; i<instances.size();i++){
			System.out.println("Saving Instance "+i);
			oos.writeObject(instances.get(i));
		}
		oos.close();
	...
	public void loadLDAInstCache(File serializedFile) throws ClassNotFoundException {
		try{
	        ...
	        InstanceList instListAux = new InstanceListCache (new SerialPipes(pipeList));
			instances = instListAux;
			Instance instAux;
			ObjectInputStream ois = new ObjectInputStream (new FileInputStream(serializedFile));
			int i=0;
				
			while((instAux = (Instance) ois.readObject()) != null){
					instances.add(instAux);
					System.out.println("Loading Instance "+i);
					i++;
			}			
	...
	public void loadLDAInstCacheMultiThread(File serializedFile) throws ClassNotFoundException {
		try{
			//Loads current instance based on Cache implementation of InstanceListCache
			// Creates Instances from a file in SEMplest format
	        ArrayList<Pipe> pipeList = this.initPipeList();
