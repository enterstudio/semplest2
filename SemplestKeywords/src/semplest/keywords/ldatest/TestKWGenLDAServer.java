...
public class TestKWGenLDAServer
{
	...
	/**
	 * This class packages the Keyword generation process using used in GenerateKeywordsMixModels for a better integration with UI in a server manner.
	 * Keeps track of multiple queries and stores information for each of them. At least 30G of memory heap required to instance one of this objects;
	 * Data will be structured in a Query Hashmap. Every time the method getCateg(data) is called,\ a new instance of a query will be generated and a
	 * string with the id of the query will be the first element in the ArrayList<String> returned to the client.
	 * 
	 * @param args
	 * @throws Exception
	 */
	...
	// Properties:
	private static final Logger logger = Logger.getLogger(TestKWGenLDAServer.class);
	 ...
	 * categories classified private TreeMap<String, Double> wordMap; //sorted map of keywords and probabilities for last run of getKW private boolean
	 * ready4kw, genericMade; // Final list of categories has been generated. private ArrayList<String> optList; private String data; //Input data for
	 * the kwgenerator; private InstanceList inferInst; //Instance of the input data; private double[] probDistInf; //Probability distribution of the
	 * last inferInst; private HashMap<String, Double> options; //Map of repeated patterns and counts private int indexFinCat; //index of the final
	...
	public TestKWGenLDAServer() throws Exception
	{
		...
		/*
		 * System.out.println("Loading Trigrams for each subcategory"); triGrams= new MultiWordCollect[nGramsSubC.length]; for (int i=0; i<
		 * nGramsSubC.length; i++){ String triPath = baseMultiWPath+nGramsSubC[i]+".txt.3"; System.out.println("Loading"+triPath); triGrams[i]= new
		 * MultiWordCollect(nGramsSubC[i],triPath); }
		 */
	...
	public List<String> getCateg(String indata) throws Exception
	{
		...
		Map<String, Double> sorted_opt = inferFromData(descModl, newQuery);
	...
	public List<String> getCateg(int index, String id) throws Exception
	{
		...
		if (index == -1)
		{
			if (query.genericMade == false)
			{
				...
				return appendID(query.optList, id);
			}
			else
			{
		...
		Map<String, Double> sorted_opt = inferFromData(subModl[query.indexFinCat], query);
	...
	private List<String> appendID(List<String> list, String id)
	{
		...
		final ArrayList<String> temp = new ArrayList<String>(list);
		returnList = (ArrayList<String>) temp.clone();
		returnList.add(0, id);
		return returnList;
	}

	// -----------------------------------------------------------------------------------------------------------------
	private Map<String, Double> inferFromData(MalletTopic model, KWGenLDAStatus query)
	{
		double[][] categInd;
		int i = 0, n;
		...
		Map<String, Double> sorted_opt = new TreeMap<String, Double>(bvcAux);
		...
		for (n = 0; n < query.topCat.length; n++)
		{
			for (int m = 0; m < n; m++)
			{
				pair[0] = query.topCat[n];
				pair[1] = query.topCat[m];
	...
	// -------------------------------------------------------------------------------------------------------------------------------
	private void selectOptions(Map<String, Double> sorted_opt, int numNEval, KWGenLDAStatus query)
	{
	...
	public static void main(String[] args) throws Exception
	{
		...
		// -----------------------------------------------------------------------------------
		String data1, data2;
		List<String> words1, words2, userIn1, keywords1, keywords2, bikwords1, trikwords1, categories1, categories2;
		...
		while (!url1.equals("exit"))
		{
			...
			// -------------------------------------------------------------------------
			/*
			 * //-------------------------URL2-------------------------------------------- // Create data from URL2 words2 = TextUtils.validHtmlWords
			 * (url2); System.out.println("Words from URL2: "+ words2.size()); data2 =""; for( String s : words2 ) { data2 = data2+" "+s;
			 ...
			 * indexes = mySentence.split(","); indexInt = new int[indexes.length];
			 * 
			 * for (int m=0; m<indexes.length;m++){ indexInt[m]=Integer.parseInt(indexes[m]); } try{//Get keywords based on user input
			 * keywords2=kwGenerator.getKw(indexInt, 100,id2); kwGenerator.killQuery(id2);//Remove Query from list keywords2.remove(0); for(String
			 * kw:keywords2){ System.out.println(kw); } }catch(Exception e){ System.out.println(e); } }
			 */// ----------------------------------------------------------------------------------------
			...
			/*
			 * System.out.println("Please, introduce your landing page 2 (type \"exit\" to close) :"); scanUrl = new Scanner(System.in); url2 =
			 * scanUrl.nextLine();
			 */
		}
