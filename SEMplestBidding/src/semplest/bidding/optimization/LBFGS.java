 ...
 * 
 * <p>
 * This code is derived from the Fortran program <code>lbfgs.f</code>. The
 * Java translation was effected mostly mechanically, with some manual clean-up;
 * in particular, array indices start at 0 instead of 1. Most of the comments
 * from the Fortran code have been pasted in here as well.
 * </p>
 * 
 * <p>
 ...
 * http://www.netlib.org/opt/lbfgs_um.shar</a>. This info is taken verbatim
 * from the Netlib blurb on the Fortran source.
 * </p>
 * 
 * <pre>
...
public class LBFGS {
   ...
   * problems) it may be advantageous to set <code>gtol</code> to a small
   * value. A typical small value is 0.1. Restriction: <code>gtol</code>
   ...
   * parameter <tt>x</tt> of <tt>lbfgs</tt>, which is modified by
   * line-search steps. A caller which wants to stop the optimization iterations
   * before <tt>LBFGS.lbfgs</tt> automatically stops (by reaching a very small
   ...
   * matrix <code>Hk0</code>, using information from the previous M steps.
   * The user specifies the number <code>m</code>, which determines the
   * amount of storage required by the routine. The user may also provide the
   * diagonal matrices <code>Hk0</code> if not satisfied with the default
   * choice. The algorithm is described in "On the limited memory BFGS method
   * for large scale optimization", by D. Liu and J. Nocedal, Mathematical
   ...
   * over these computations, reverse communication is used. The routine must be
   * called repeatedly under the control of the parameter <code>iflag</code>.
   * 
   * The steplength is determined at each iteration by means of the line search
   * routine <code>mcsrch</code>, which is a slight modification of the
   ...
   *        it must be set by the user to contain the components of the gradient
   *        <code>g</code> at the point <code>x</code>.
   * 
   * @param diagco Set this to <code>true</code> if the user wishes to provide
   *        the diagonal matrix <code>Hk0</code> at each iteration. Otherwise
   *        it should be set to <code>false</code> in which case
   ...
   *        A return with <code>iflag &lt; 0</code> indicates an error, and
   ...
   *        <code>g</code>. On a return with <code>iflag = 2</code>, the
   *        user must provide the diagonal matrix <code>Hk0</code>.
   * 
   ...
   * <li> Rounding errors prevent further progress. There may not be a step
  ...
  public static void lb1(int[] iprint, int iter, int nfun, double gnorm, int n,
      int m, double[] x, double f, double[] g, double[] stp, boolean finish) {
    int i;
    ...
    if (iter == 0) {
      ...
      if (iprint[1 - 1] != 0) {
        ...
        if (iprint[2 - 1] > 1 && finish)
          System.err.println("\ti\tnfn\tfunc\tgnorm\tsteplength");
        System.err.println("\t" + iter + "\t" + nfun + "\t" + f + "\t" + gnorm
            + "\t" + stp[0]);
      }
   ...
   * ways to carry out this operation; this code is a straight translation from
   * the Fortran.
   */
